  ...
  <VDropdown min-height="10rem" dropdown-class="dark:bg-primary-dark-1 dark:!border-primary-dark-2/50" v-bind="$attrs" max-height="24rem" class="lg:flex hidden " @close="emit('update:modelValue',false)" :on="modelValue">
    ...
    <template v-slot:content>
      <div class="flex justify-between items-center p-1">
        ...
        <font-awesome-icon  size="1.4rem" @click="emit('update:modelValue',false)" class="text-gray-600 cursor-pointer relative z-[100] w-1.4 dark:text-primary-dark-3" icon="fa-solid fa-xmark" />
      </div>
      ...
      <div v-else class="my-0.5 max-h-[19.5rem] overflow-y-auto scroller">
        <template v-if="searchResult.length>0">
          ...
          <router-link  :to="{name:item.link}" v-for="item in searchResult" class="search-result-item">
...
const {searchResult,searchContent,searchResultFlag}=useDashboardStore()
const {searchHandler}=useSearch(props,emit)
